...
public class Executor {
	...
	private static boolean initializeCustomContextFactory = false;
	private static CustomContextFactory customContextFactory = null;

	/**
	 * This method initializes the customContextFactory
	 * 
	 */
	private static void initializeCustomContextFactory() {

		if (!initializeCustomContextFactory) {
			customContextFactory = new CustomContextFactory();
			initializeCustomContextFactory = true;
		}
	...
	public static Context createContext(java.net.URL codeSource, UserAgentContext ucontext) {
		...
		initializeCustomContextFactory();
		if (customContextFactory != null) {
			Context prev1 = Context.getCurrentContext();
			Context ctx1 = Context.enter();
			// ctx1.setOptimizationLevel(ucontext.getScriptingOptimizationLevel());
			if (prev1 == null) {
				...
				ctx1.setSecurityController(new SecurityControllerImpl(codeSource, ucontext.getSecurityPolicy()));
			}
			return ctx1;
		} else {
